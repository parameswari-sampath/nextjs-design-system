...
export default function AllTestsPage() {
  ...
  const copyAccessCode = async (accessCode: string) => {
    try {
      ...
      showToast("destructive", "Failed to copy access code");
    }
  };
  ...
  const fetchAssessments = async () => {
    try {
      ...
      showToast(
        "destructive",
        "Failed to fetch assessments. Please check your connection."
      );
    } finally {
      setLoading(false);
    }
  ...
  const deleteAssessment = async () => {
    ...
    try {
      ...
      showToast(
        "destructive",
        "Failed to delete assessment. Please check your connection."
      );
    } finally {
  ...
  const togglePublishStatus = async (assessment: Assessment) => {
    try {
      ...
      if (response.ok) {
        ...
        setAssessments((prev) =>
          prev.map((a) =>
            a.id === assessment.id ? { ...a, is_published: !a.is_published } : a
          )
        );
        ...
        showToast(
          "success",
          data.message || `Assessment ${action}ed successfully`
        );
      } else {
        const errorData = await response.json();
        ...
        const errorMsg =
          errorData.error ||
          `Failed to ${action} assessment. Please try again.`;
      ...
      showToast(
        "destructive",
        "Failed to update assessment status. Please check your connection."
      );
    } finally {
  ...
  useEffect(() => {
    console.log("📝 All Tests page mounted/updated", {
      currentPage,
      itemsPerPage,
    });
    fetchAssessments();
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Modal isOpen={showViewModal} onClose={closeViewModal} size="xl">
        <ModalHeader>
        ...
        <ModalBody>
          {assessmentDetails && (
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-[var(--color-muted-foreground)]">
                        {assessmentDetails.duration_minutes} minutes
                      </p>
                    </div>
                    <div>
                      <span className="font-medium text-[var(--foreground)]">
                      ...
                      <p className="text-[var(--color-muted-foreground)]">
                        {assessmentDetails.total_questions}
                      </p>
                    </div>
                    <div>
                      <span className="font-medium text-[var(--foreground)]">
                    ...
                    <div>
                      ...
                      <p className="text-[var(--color-muted-foreground)]">
                        {assessmentDetails.max_attempts}
                      </p>
                    </div>
                    <div>
                      <span className="font-medium text-[var(--foreground)]">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="flex items-center justify-between p-3 bg-[var(--color-secondary)] rounded-[var(--radius)]">
                    ...
                    <Badge
                      variant={
                        assessmentDetails.shuffle_questions
                          ? "success"
                          : "secondary"
                      }
                    >
                  ...
                  <div className="flex items-center justify-between p-3 bg-[var(--color-secondary)] rounded-[var(--radius)]">
                    ...
                    <Badge
                      variant={
                        assessmentDetails.shuffle_options
                          ? "success"
                          : "secondary"
                      }
                    >
                  ...
                  <div className="flex items-center justify-between p-3 bg-[var(--color-secondary)] rounded-[var(--radius)]">
                    ...
                    <Badge
                      variant={
                        assessmentDetails.show_results_immediately
                          ? "success"
                          : "secondary"
                      }
                    >
                  ...
                  <div className="flex items-center justify-between p-3 bg-[var(--color-secondary)] rounded-[var(--radius)]">
                    ...
                    <Badge
                      variant={
                        assessmentDetails.allow_retake ? "success" : "secondary"
                      }
                    >
              ...
              {/* Meta Information */}
              <div>
                <h4 className="text-md font-medium text-[var(--foreground)] mb-2">
                  Created Information
                </h4>
                <div className="text-sm text-[var(--color-muted-foreground)] space-y-1">
                  <p>
                    <strong>Created by:</strong> {assessmentDetails.created_by}
                  </p>
                  <p>
                    <strong>Created at:</strong>{" "}
                    {new Date(assessmentDetails.created_at).toLocaleString()}
                  </p>
                  <p>
                    <strong>Last updated:</strong>{" "}
                    {new Date(assessmentDetails.updated_at).toLocaleString()}
                  </p>
                </div>
              </div>
            </div>
